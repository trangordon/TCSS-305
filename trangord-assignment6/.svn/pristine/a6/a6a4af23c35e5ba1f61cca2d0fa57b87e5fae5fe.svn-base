...
public class Board {
    ...
    private void checkRows() {
        ...
        if (!completeRows.isEmpty()) {
            ...
            //Moved the fire property change into the if so only fires when lines are cleared.
            myPCS.firePropertyChange("lines cleared", null,
                                     completeRows.toArray(new Integer[completeRows.size()]));
        }
        //Added a fire property change to notify when block is placed 
        myPCS.firePropertyChange("piece placed", null, null);
